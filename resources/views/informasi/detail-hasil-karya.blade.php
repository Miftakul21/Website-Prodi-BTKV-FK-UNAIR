...
@section('title', $hasil_karya_title)
@push('css')
<!-- link css -->
{!! SEO::generate() !!}
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 order-1 col-12 mb-3">
                        <div class="container-informasi">
                            <div class="d-flex justify-content-between">
                                <div>
                                    <div><i class="bi bi-person"></i> {{$hasil-karya_editor }}</div>
                                    <div><i class="bi bi-calendar"></i> {{\Carbon\Carbon::parse($hasil-karya_date)->format('M, d Y')}}</div>
                                </div>
                                <div>
                        ...
                        <h1 class="title-news">{{$hasil-karya_title}}</h1>
                        <div class="d-flex justify-content-between align-items-center gap-2" style="width: 160px;">
                            <span class="category">{{$hasil-karya_category}}</span>
                            ...
                            <div class="dropdown d-inline-block">
                                ...
                                <ul class="dropdown-menu dropdown-menu-end p-2" aria-labelledby="shareDropdown" style="min-width: auto;">
                                    <div class="d-flex gap-2 px-2">
                                        ...
                                        <!-- Twitter / X -->
                                        <a href="https://twitter.com/intent/tweet?url={{ urlencode(request()->fullUrl()) }}&text={{ urlencode($hasil-karya_title ?? '') }}" target="_blank" class="btn btn-outline-info btn-sm text-info" title="Twitter">
                                            <i class="bi bi-twitter"></i>
                                        </a>
                                        <!-- WhatsApp -->
                                        <a href="https://wa.me/?text={{ urlencode(($hasil-karya_title ?? '') . ' ' . request()->fullUrl()) }}" target="_blank" class="btn btn-outline-success btn-sm text-success" title="WhatsApp">
                    ...
                    <div class="col-md-6 order-2 col-12 mt-md-none mt-xl-none">
                        <div class="container-thubmnail-image">
                            <img src="{{ asset('storage/'.$hasil-karya_thumbnail) }}" alt="gambar-artikel" class="image-news img-fluid">
                        </div>
                    </div>
                </div>
            </div>
...
<!-- pagination section -->
<section class="pagination-hasil-karya mb-3">
    <div class="container">
        <div class="d-flex justify-content-center align-items-center mb-3">
            <span class="text-muted text-center fw-bold" style="font-size: 12px;">Artikel Hasil Karya Lainnya</span>
        </div>
        <div class="d-flex flex-column flex-md-row justify-content-center flex-wrap gap-1">
            @forelse($hasil-karya_lainnya as $artikel)
            <div class="card card-hover">
                <div class="card-body p-3">
                    <h6 class="date-artikel-pagination">{{\Carbon\Carbon::parse($artkel->hasil-karya_date)->format('M, d Y')}} September, 22 2025</h6>
                    <h3 class="title-artikel-pagination">
                        {{Str::limit($artikel->hasil-karya_title), 90}}
                    </h3>
                </div>
            </div>
            @empty
